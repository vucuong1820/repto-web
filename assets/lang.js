...
(function () {
  ...
  function altHref(l) {
    ...
    var href = n && n.getAttribute('href');
    if (!href) return null;
    // hreflang URLs are absolute (SEO); link to the same-origin path so the
    // dropdown works on localhost/preview too, not only on production.
